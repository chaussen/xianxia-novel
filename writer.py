...
def build_user_message(chapter_num: int) -> str:
    return f"""请根据上方所有参考材料，为**第{chapter_num}章**写完整正文草稿。

写作要求（再次确认）：
- 直接输出正文，从 `# 第{chapter_num}章：[章节标题]` 开始
- 严格遵守三层语言架构：对白/独白无现代词；旁白无物理术语；物理律名仅在括号注里
- 顾青对话与独白绝不出现「能量、变量、系统、数据、效率、参数」等现代词
- 物理原理必须来自《天工理略》某残页或现场观察，不能凭空想到
- 格物场景必须经过至少一次真实失败，再修正成功
- 章首先附【当前状态监测】，章末附〔账本余页〕
- 与上一章的叙事状态和人物位置自然衔接，不重复交代已知信息
- 按需使用向心力/旋气场景或CHAPTER_LOG中尚未使用的物理原理（不重复已用原理）
